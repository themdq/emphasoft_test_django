...
class SearchResultsView(ListView):
    ...
    def get_queryset(self):
        ...
        cache.set('num', query)
        # case 1: a room is booked before the check_in date, and checks out
        # after the requested check_in date
        case_1 = Booking.objects.filter(
            checkin_date__lte=checkin,
            checkout_date__gte=checkin)

        # case 2: a room is booked before the requested check_out date and
        # check_out date is after requested check_out date
        case_2 = Booking.objects.filter(
            checkin_date__lte=checkout,
            checkout_date__gte=checkout)

        case_3 = Booking.objects.filter(
            checkin_date__gte=checkin,
            checkout_date__lte=checkout)
...
def book(request):
    ...
    room_num = Room.objects.filter(
        Q(room_no__icontains=int(request.GET.get('numb'))))[0]
